...
const $$PlatformCards = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const categoryData = await fetchCategoryData();
  return renderTemplate`<!-- Shopify Card Section  -->${maybeRenderHead()}<div${addAttribute(`${classButton || "platform-content"} space-y-5 sm:space-y-0 grid sm:grid-cols-2 xl:grid-cols-3 mt-[30px] sm:gap-6 sm:w-full content-center`, "class")}${addAttribute(`display:${Display || "none;"}`, "style")}${addAttribute(dataPlatform, "data-platform")}> ${categoryData.map((item) => renderTemplate`${renderComponent($$result, "Card", $$Card, { "Icon": item.icon_image, "slug": item.slug, "headTag": item.post_category_name, "paragraphTag": item.description, "review": item.review, "hrefLink": item.header_url, "rating": item.rating, "newLabel": item.tag, "CTAButton": item.cta_button, "ReviewLink": item.review_url, "HeaderLinkTarget": item.header_url_target, "ReviewLinkTarget": item.review_url_target })}`)} </div>`;
